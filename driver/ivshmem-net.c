...
#define IVSHMEM_CFGFLAG_INTX		0x1
...
static void ivshm_net_notify_tx(struct ivshm_net *in, unsigned int num)
{
	...
	if (vring_need_event(evt, new, old)) {
		writel(IVSHM_NET_VECTOR_TX_RX, &in->ivshm_regs->doorbell);
		in->stats.tx_notify++;
	}
}
...
static int ivshm_net_poll(struct napi_struct *napi, int budget)
{
	...
	while (received < budget) {
		...
		data = ivshm_net_desc_data(in, &in->rx, IVSHM_NET_REGION_RX,
					   desc, &len);
		if (!data) {
			   ...
			   const struct pci_device_id *pci_id)
{
	...
	for (region = 0; region < 2; region++) {
		...
		shmaddr[region] = dword;
		pci_read_config_dword(pdev, cap_pos + 4, &dword);
		shmaddr[region] |= (u64)dword << 32;

		cap_pos = vendor_cap + JAILHOUSE_CFG_SHMEM_SIZE1 + region * 16;
		pci_read_config_dword(pdev, cap_pos, &dword);
		shmlen[region] = dword;
		pci_read_config_dword(pdev, cap_pos + 4, &dword);
		shmlen[region] |= (u64)dword << 32;

		if (!devm_request_mem_region(&pdev->dev, shmaddr[region],
					     shmlen[region], DRV_NAME))
			return -EBUSY;

		shm[region] = devm_memremap(&pdev->dev, shmaddr[region],
					    shmlen[region], MEMREMAP_WB);
		if (!shm[region])
			return -ENOMEM;

		dev_info(&pdev->dev, "%s memory at %pa, size %pa\n",
			 region == IVSHM_NET_REGION_TX ? "TX" : "RX",
			 &shmaddr[region], &shmlen[region]);
	}
	...
	in = netdev_priv(ndev);
	in->ivshm_regs = regs;
	in->shm[IVSHM_NET_REGION_TX] = shm[IVSHM_NET_REGION_TX];
	in->shm[IVSHM_NET_REGION_RX] = shm[IVSHM_NET_REGION_RX];
	in->shmaddr[IVSHM_NET_REGION_TX] = shmaddr[IVSHM_NET_REGION_TX];
	in->shmaddr[IVSHM_NET_REGION_RX] = shmaddr[IVSHM_NET_REGION_RX];
	in->shmlen = min(shmlen[IVSHM_NET_REGION_TX],
			 shmlen[IVSHM_NET_REGION_RX]);
	in->peer_id = !id;
	in->pdev = pdev;
	...
	if (pdev->msix_enabled) {
		...
		ret = request_irq(pci_irq_vector(pdev, IVSHM_NET_VECTOR_STATE),
		...
		ret = request_irq(pci_irq_vector(pdev, IVSHM_NET_VECTOR_TX_RX),
		...
		pci_write_config_byte(pdev, vendor_cap + JAILHOUSE_CFG_FLAGS,
				      IVSHMEM_CFGFLAG_INTX);
	}

	pci_set_master(pdev);

	in->rstate = shm[IVSHM_NET_REGION_TX];

	/* First make sure that rstate writing is disabled. */
	writel(0, &in->ivshm_regs->rstate_write_lo);

	writel(0x0, &in->ivshm_regs->rstate_write_hi);
	writel(0x0 | IVSHMEM_RSTATE_WRITE_REGION1 | IVSHMEM_RSTATE_WRITE_ENABLE,
	       &in->ivshm_regs->rstate_write_lo);
...
err_request_irq2:
	free_irq(pci_irq_vector(pdev, IVSHM_NET_VECTOR_STATE), in);
err_request_irq:
...
static void ivshm_net_remove(struct pci_dev *pdev)
{
	...
	if (pdev->msix_enabled) {
		free_irq(pci_irq_vector(pdev, IVSHM_NET_VECTOR_STATE), in);
		free_irq(pci_irq_vector(pdev, IVSHM_NET_VECTOR_TX_RX), in);
	} else {
		vendor_cap = pci_find_capability(pdev, PCI_CAP_ID_VNDR);
		pci_write_config_byte(pdev, vendor_cap + JAILHOUSE_CFG_FLAGS,
				      0);
